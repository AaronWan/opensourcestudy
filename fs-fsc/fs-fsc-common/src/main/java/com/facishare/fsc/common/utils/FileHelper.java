...
public class FileHelper {
    ...
    public static DocumentFormat getDocumentFormat(String extension) {
        ...
        if (mime == null) return DocumentFormat.Unknown;
        int index = mime.indexOf('/');
        if (index < 0) return DocumentFormat.Unknown;
        String prefix = mime.substring(0, index);
        switch (prefix) {
            case "audio":
                return DocumentFormat.Audio;
            case "image":
                if (mime.equalsIgnoreCase( "image/bmp") || mime.equalsIgnoreCase("image/gif") || mime.equalsIgnoreCase("image/jpeg") || mime.equalsIgnoreCase("image/png"))
